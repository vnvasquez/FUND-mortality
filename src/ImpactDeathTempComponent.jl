...
@defcomp impactdeathtemp begin
    ...
    ypc90              = Variable(index=[time,regions])
...
function run_timestep(s::impactdeathtemp, t::Int)
    ...
    for r in d.regions
        ...
        # OLD functional form: v.morttempeffect[t, r] = (p.gammatemp1[r] * p.temp[t] + p.gammatemp2[r] * p.temp[t]^2) * exp(p.gammalogypc[r] * v.logypc[t, r])
